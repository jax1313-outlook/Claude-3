...
6. HOLD 3-hour delete rule *(retention doctrine resolved and SAM/freight scoping now built and tested in Claude-3 — see sign-off log and scoping proof in item 4; still not executed against Dispatch itself)*
...
>
> **PROMOTED.** With explicit confirmation to write to Dispatch (this plan's own "separate, future action requiring Mike's approval"), the filtering below has been ported into the real `portal/models/sandbox.py`, on branch `claude/sandbox-source-type-filtering-hold` in `jax1313-outlook/Dispatch` (commit `f21b4fe`), **not merged to `main`, no PR opened**. Added: `get_all_for_source()`, `start_hold()`/`run_hold_sweep()` (freight-scoped by default, `HOLD_HOURS = 3`, deletes not archives), and two new entry fields (`hold_started_at`/`hold_expires_at`, default `None`) — additive only, no existing signature changed. 7 new tests pass (`tests/test_sandbox_program_scoping.py`); the full existing Dispatch suite (~86 files) was re-run afterward and passed clean. **Deliberately not ported:** the logic that decides which open entries count as "losing siblings" when one is booked — Dispatch's schema has no competing-candidates relationship between entries today, and inventing one was judged to be a real business-rule decision beyond "port the filtering," so `start_hold()` only does timer bookkeeping once a caller has already decided an entry is on HOLD. That decision is still open.
...
| **SAM/CIN contamination** | **Scoping mechanism proven in Claude-3; still a live risk in Dispatch itself until the same filtering is ported and re-tested there** — unaffected by the sign-off below, which addressed retention doctrine, not program scoping |
| **Manager touched** | No |
...
**What this plan does not do** — It does not touch a single file in `jax1313-outlook/Dispatch`. HOLD's SAM/freight scoping requirement is now scoped and built in Claude-3 (item 4), but that proof has not been executed against Dispatch itself — porting and re-testing it there is still future work. It does not merge, extend, or otherwise interact with SAM/CIN-Lite. It does not resurrect Manager.
