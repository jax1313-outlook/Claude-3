...
| **M4** | #98 | Enriched Email Helper's drafted emails with real closeout data (pickup/delivery/rate/invoice/POD) instead of a generic placeholder. | Flagged, then **resolved**: the customer email includes the same rate/invoice figures as the broker email. Explicitly confirmed as intentional ("leave customer email as is") — not a business-sensitivity gap after all, no code change needed. |
