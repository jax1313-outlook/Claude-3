...
| D11 *(new)* | **RESOLVED** | For Dispatch's purposes, **Customer, Shipper, and Broker are the same entity/role** — not three distinct parties with different information needs. | Retroactively resolves §17's M4 business-sensitivity flag (identical rate/invoice figures in the broker and customer emails, PR #98) — there was never an information-asymmetry concern, because there's no separate party to withhold the figures from. Also bears on D2 (§6): `Load`'s separate `customer` and `broker_shipper` fields, and Email Helper's two separate drafted emails (`_draft_broker_email()`/`_draft_customer_email()`), currently model these as two distinct recipients. **Not changed by this decision alone** — flagged as a real simplification opportunity (one recipient/contact concept instead of two) rather than unilaterally collapsing the schema or the Email Helper draft/send flow without being asked to. |
...
| **M4** | #98 | Enriched Email Helper's drafted emails with real closeout data (pickup/delivery/rate/invoice/POD) instead of a generic placeholder. | Flagged, then **resolved by D11** (§6): the customer email includes the same rate/invoice figures as the broker email. Turns out there was never a real information-asymmetry concern — Customer/Shipper/Broker are the same entity/role for Dispatch's purposes, so no code change needed. |
