...
| D13 *(new)* | **RESOLVED — EXERCISES A PREVIOUSLY CONDITIONAL OPTION** | Driver PIN Cards are a **Library-managed asset**: issued Mike-only, no self-registration, stored under the same data root `library.json` uses. `portal/models/identity.py`'s module docstring had explicitly left "PIN records in Library" conditional on Mike's decision (its own cited Section 11) rather than a default — this instruction ("Create Driver PIN Registry as a Library-managed asset") is that decision, exercised for the Driver role only. Authority's own PIN registry (`identity.py`) is untouched, still lives outside Library, and this does not reopen or reconsider that choice. | Implemented in `portal/models/driver_pin_registry.py` (§21) — kept as its own file rather than folded into `library.py`'s generic `add_record()` schema, for the same reason `identity.py` gave for keeping Authority's PIN out of that schema in the first place: a credential is not a "record" with name/content/metadata/status. |
...
One item from §18's three-item list remains fully unbuilt: Route Risk's *live* data shape. §21 below builds an honest stub for it as part of a different, explicitly-requested feature.

---

## 21. Driver PIN Registry — Build Report

Authorized explicitly via a detailed 8-point IMPLEMENTATION TARGET: "Create Driver PIN Registry as a Library-managed asset," with Driver Portal auth (Phone Number + PIN), COMI lookup, Route Risk lookup, SMS/email contact retrieval, Recovery Word validation, and Active/Inactive status control, explicitly scoped as "intentionally lightweight," "not enterprise identity management," "not complex user-role frameworks," and "Mike remains final authority." Built on branch `claude/driver-pin-registry`, PR #101, against `main` @ `a115162` (post-Operations Feed).

**A judgment call flagged, then routed around rather than left blocking:** before this task arrived, a separate "Build Route Risk next" request had been paused on an explicit question -- Vision doc #3v2 §10 defines Route Risk as a *live* predictive capability over traffic/weather/construction, none of which exist in Dispatch's tech stack, and the user dismissed the clarifying question without answering it. This task's own requirement #5 ("Route Risk lookup capability") revives that exact dependency gap. Resolved here without re-blocking the whole PIN Registry build on it: `dispatch/route_risk.py::get_route_risk(load_id)` is built as an honest stub that always returns a clearly-labeled "not yet available" result -- never a fabricated risk level -- matching the same do-not-invent-facts principle already enforced for Publisher and the deterministic rule modules. It defines the target shape and wiring point for a real engine; building that engine (which data source, what it costs, whether it fits "lightweight") is still an open, unmade decision, not silently resolved by this stub.

**A second precedent worth naming explicitly:** `portal/models/identity.py`'s own module docstring said Authority's PIN storage choice for Library was "conditional on Mike's decision (Section 11), not a default." This task IS that decision, made now, for the Driver role. Recorded as **D13** in the decision register: *Driver PIN Cards are a Library-managed asset (stored under the same root as `library.json`), issued and managed Mike-only, with no self-registration path -- exercising the option identity.py left open, not overriding it.*

### Requirement-by-requirement

1. **Driver PIN Card structure** (`portal/models/driver_pin_registry.py`): one card per driver, linked by `driver_id` to the existing `drivers` table (name/phone/license stay there as the single source of truth -- never duplicated). Fields: `status` (active/inactive), `pin_hash`, `recovery_word_hash` (both werkzeug-hashed, mirroring `identity.py`), lockout fields (`failed_attempt_count`/`locked_until`, same 5-attempts/15-minutes as Authority), timestamps. Stored under `get_memory_dir()` -- the same directory `library.json` lives in -- so the card is a Library asset on disk, not just a UI fiction. Kept as its own file rather than folded into `library.py`'s generic `add_record()` schema, for the same reason `identity.py` gave for keeping Authority's PIN out of Library's generic schema: a credential is not a "record" with name/content/metadata/status.
2. **CRUD within Library**: `create_pin_card` / `get_pin_card` / `list_pin_cards` / `reset_pin` / `set_recovery_word` / `set_status` / `delete_pin_card`, all Mike-only by the same route-layer convention `identity.py` uses (no general create-user path). A new **Driver PIN Registry** section on the existing `/library` page (issue/reset/set recovery word/activate/deactivate/delete), backed by five new endpoints under `/api/driver-pin/*` (PIN-gated, same as every other `/api/*` route).
3. **Driver Portal auth, Phone Number + PIN** (`portal/routes/driver_portal.py`, `/driver/*`): its own session key, `session["driver_id"]`, distinct from Authority's `session["user_id"]` and the stakeholder portal's HMAC token -- a driver session can never satisfy the Authority gate or vice versa (verified against the *real* `DISPATCH_PIN` gate in tests, not just the `TESTING` default). Exempted from `portal/app.py`'s Authority gate the same way `stakeholder` already is, but with a genuine login of its own (a blueprint-level `before_request` checking `session["driver_id"]`), not a token link. Phone number is looked up live from the driver record at login time (`store.get_driver_by_phone`, new), never copied into the PIN card -- one source of truth for the phone number, not two.
4. **COMI lookup**: the dashboard shows each active load's `email_helper` package status (DRAFT/REVIEWED/SUBMITTED) as a glance, not an editable surface -- Driver-First Doctrine's cognitive-load-reduction principle applied literally: a driver gets "what's the state of communications on my load," not an email editor.
5. **Route Risk lookup**: see the flagged decision above -- an honest "not yet available" stub, `dispatch/route_risk.py`.
6. **SMS/email contact retrieval**: broker contact (name/phone/email) for the load's `broker_shipper`, via the existing `broker_contacts` table, plus Level 1 Transport's own dispatch/reviewer email (`cin_lite.email_delivery.reviewer_address()`, already the real configured address). No SMS number surfaced for Level 1 Transport itself -- there is no SMS channel or phone-number config anywhere in this codebase (Email API is the only configured communication channel per CLAUDE.md's tech stack), so this deliberately retrieves real, already-known contact data rather than inventing a channel that doesn't exist.
7. **Recovery Word validation**: `verify_recovery_word` / `reset_pin_with_recovery_word`, exposed at `/driver/forgot-pin` -- the one self-service action a driver gets (a forgotten PIN is exactly the moment they can't be logged in already). An inactive card cannot self-recover either, matching `verify_login`'s own active-status check.
8. **Active/Inactive status control**: `set_status()`, deliberately independent of the driver's own roster `status` field on the `drivers` table -- Mike can suspend Driver Portal access without touching employment status, or the reverse. Flagged as an explicit design choice, not silently merged into one field.
